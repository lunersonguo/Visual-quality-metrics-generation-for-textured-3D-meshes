...
In such cases, we used sorting method to reduce comparisons number and improve the efficiency.  In details, we firstly divided 20 distorted models into 5 groups by their distortion types (compression, simplification, smoothing, JPEG and Sub-sampling). In each group, we sorted the 4 distorted models by the increasing strengths of distortions. According to the sorting method, the quality distances among groups are closer than those inside each group.
