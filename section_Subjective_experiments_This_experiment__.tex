...
Since the task of each observer in paired comparisons is to provide a subjective selection of one distorted model, which is closer to the reference, the number of comparisons (“rounds”) determines the reliability of subjective quality opinion. Ideally, when \textit{N} distortions are evaluated, the total number of rounds is $C^N_2$, which means to show all the possible combinations of two distorted models from \textit{N} distortions. However when \textit{N} is large (e.g., 20 in our study), the number of pairs become too many (e.g., 190 in our study) to be feasible \cite{Lee_2011}, which means each subject may spend much time on some redundant comparisons. For instance, two models with same distortion but different strengths, one with lower distorting strength is statistically preferred to the other. The total comparisons of all the possible combinations are lack of efficiency, and the redundant comparisons may distract subjects from the valuable ones.
