...
\end{equation}
where $n$ is the number of scales (3 in our study). Finally, Minkowski pooling method was used to pool all the local distortion measures into one global score $G\Delta$:
\begin{equation}
G\Delta =(\frac{\sum_{i=1}^N{\delta_M(v)^p}}{N})^\frac{1}{p}
\end{equation}
where $N$ is the total  vertices number on $M_d$, typical $p$ values are from 2 to 4, and via our trails and comparisons, we determined to use 2(Root Mean Square).\\
Since this metric measures the amount of mean curvature variances over small regions between two meshes. It concentrates more on the variances of local roughness, and less on the global slight variation, and hence partially approximates HVS sensitivity toward geometric distortions. The performances of this new metric and other current visual quality ones are evaluated in the next section.  