...
\end{itemize}
For the criteria, we selected 5 commonly used geometric and textural distortions, solely applied each of them to each model in 4 different strengths based our empirical decisions, and generated 20 distorted models for one reference model(one mesh or texture is processed by one single strength of the algorithm). We used following processing operations to generate all the distortions:\\
for each mesh,
\begin{itemize}
\item \textbf{Compression}:  we used common lossy process of  compression algorithms to generate uniform geometric quantization.
\end{itemize}
\begin{itemize}
\item  \textbf{Simplification}: We apply quadric based mesh decimation method \cite{Garland_1997} to generate the simplified models in this study.
\end{itemize}
\begin{itemize}
\item \textbf{Smoothing}:We consider Laplacien smoothing algorithm \cite{Steinbrecher_2008}.
\end{itemize}
