...
\begin{table} 
    \begin{tabular}{ c c c c }
        \textbf{Reference Model} & \textbf{Observers Number} & \textbf{Kendall's W} & \textbf{p-value} \\ 
...
\end{table}
Table 1 shows the details that overall Kendall’s \textit{W} coefficients are least bigger than 0.71, implying that for each reference, all the subjects are quite unanimous about sorting the visual qualities of its 20 distorted models, and sufficiently small p-values shows the high reliability of the \textit{W} coefficients. These high consistencies of subjective opinion support us to continue exploiting all the results from the experiments.\\
